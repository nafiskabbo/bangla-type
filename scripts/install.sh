...
echo "=== Installing $APP_NAME (does not remove the main BanglaType.app from Xcode) ==="
...
# Remove only this variant's previous install (not com.banglatype.inputmethod.BanglaType)
rm -rf "$INSTALL_DIR/$APP_NAME.app" 2>/dev/null || true
rm -rf "$INSTALL_DIR/Lekho.app" 2>/dev/null || true
...
echo "  3. Click '+' → search for 'BanglaType IME' → select 'BanglaType IME'"
